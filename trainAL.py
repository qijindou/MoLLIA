...
from queryBaseline import get_query_index, rand_select, get_ns_query_index, get_cs_query_index
...
class ClassificationTrainer(object):
    ...
    def run(self, data_loader, al_model, optimizer, scheduler, stage,
            i_al, mode=ModeType.EVAL, al_weights=None, negative_labels=None):
        predict_labels = []
        standard_labels = []
...
def get_unlabeled_prediction(conf, args, al_model, optimizer, train_unlabel_dataloader):
    al_model_file_prefix = f"{args.output_dir}{conf.checkpoint_dir}/{args.n_train}_{args.al_model}_"
    load_checkpoint(f"{al_model_file_prefix}train_best", al_model, optimizer)
    al_model.eval()
    ...
    with torch.no_grad():
        for batch in train_unlabel_dataloader:
            ...
            logits = al_model(**input).logits
            result = F.softmax(logits, dim=-1).cpu().tolist()
            predict_unlabel.extend(result)
    return np.array(predict_unlabel)

def get_embedding(conf, args, al_model, train_unlabel_dataloader, train_al_dataloader):
    al_model.eval()
    labeled_embeddings = []
    ...
    with torch.no_grad():
        for batch in train_al_dataloader:
            input = {k: v.to(conf.device) for k, v in batch.items()}
            outputs = al_model(**input)
            cls_embeddings = outputs.hidden_states[-1][:, 0, :].cpu().numpy()
            labeled_embeddings.append(cls_embeddings)
    
        for batch in train_unlabel_dataloader:
            input = {k: v.to(conf.device) for k, v in batch.items()}
            outputs = al_model(**input)
            cls_embeddings = outputs.hidden_states[-1][:, 0, :].cpu().numpy()
            unlabeled_embeddings.append(cls_embeddings)

    return np.concatenate(labeled_embeddings, axis=0), np.concatenate(unlabeled_embeddings, axis=0)
...
def train(conf, args):
    ...
    for i_al in range(1, 1+args.num_al):
        ...
        for key in performance_dict:
            result_file.write(json.dumps({f"{key}_performance": performance_dict[key]})+"\n")
        result_file.close()
        ...
        if i_al == args.num_al or AL_last:
            ...
            for key in performance_dict:
                final_result_file.write(json.dumps({f"{key}_performance": performance_dict[key]})+"\n")
        ...
        elif args.sampling == "NoiseStability":
            sample_ls, train_unlabel_ls = get_ns_query_index(conf, args, sample_ls, train_unlabel_ls, al_model, train_unlabel_dataloader)
        elif args.sampling == "CoreSet":
            labeled_embeddings, unlabeled_embeddinds = get_embedding(conf, args, al_model, train_unlabel_dataloader, train_al_dataloader)
            sample_ls, train_unlabel_ls = get_cs_query_index(conf, args, sample_ls, train_unlabel_ls, labeled_embeddings, unlabeled_embeddinds)
        else:
            prob_X_Y = get_unlabeled_prediction(conf, args, al_model, optimizer, train_unlabel_dataloader)
            sample_ls, train_unlabel_ls = get_query_index(logger, conf, args, i_al, sample_ls, train_unlabel_ls, prob_X_Y)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--sampling", default="random", choices=["random", "max_entropy", "NoiseStability", "CoreSet"], 
